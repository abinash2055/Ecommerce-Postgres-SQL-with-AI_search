...
export const createProduct = catchAsyncErrors(async (req, res, next) => {
    ...
    const product = await database.query(`INSERT INTO products (name, description, price, category, stock, images, created_by) VALUES ($1, $2, $3, $4, $5, $6, $7) RETURNING *`,
        [name, description, price / 153, category, stock, JSON.stringify(uploadedImages), created_by]);
...
export const fetchAllProducts = catchAsyncErrors(async (req, res, next) => {
    ...
    const newProductsQuery = `SELECT p.*, COUNT(r.id) AS review_count FROM products p LEFT JOIN reviews r ON p.id = r.product_id WHERE p.created_at >= NOW() - INTERVAL '30 days' GROUP BY p.id ORDER BY p.created_at DESC LIMIT 8`;
    ...
    res.status(200).json({ success: true, products: result.rows, totalProducts, newProducts: newProductsResult.rows, topRatedProducts: topRatedResult.rows });
});
...
export const updateProduct = catchAsyncErrors(async (req, res, next) => {
    ...
    const result = await database.query(`UPDATE products SET name = $1, description = $2, price = $3, category = $4, stock = $5 WHERE id = $6 RETURNING *`, [name, description, price / 153, category, stock, productId]);
