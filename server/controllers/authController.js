...
export const register = catchAsyncErrors(async (req, res, next) => {
    ...
    if (isAlreadyRegistered.rows.length > 0) {
        return next(new ErrorHandler("User already registered with this email....", 404))
    }
...
export const logout = catchAsyncErrors(async (req, res, next) => {
    res.status(200).cookie("token", "", { expires: new Date(Date.now()), httpOnly: true }).json({ success: true, message: "Logged out successfully...." });
})
...
export const forgotPassword = catchAsyncErrors(async (req, res, next) => {
    ...
    try {
        await sendEmail({ email: user.email, subject: "AI Ecommerce Password Recovery", message });
...
export const updateProfile = catchAsyncErrors(async (req, res, next) => {
    ...
    if (req.files && req.files.avatar) {
        ...
        const newProfileImage = await cloudinary.uploader.upload(avatar.tempFilePath, {
            folder: "AI_Ecommerce_Avatars", width: 150, crop: "scale"
        });
