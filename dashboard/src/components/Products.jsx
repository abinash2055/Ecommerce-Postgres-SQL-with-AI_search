    ...
    <>
      <main className="p-[10px] pl-[10px] md:pl-[17rem] w-full">
        ...
        <div className="flex-1 md:p-6">
          ...
          <p className="text-sm text-gray-600 mb-6">Manage all your Products</p>
          ...
          <div className="p-4 sm:p-8 bg-gray-50">
                ...
                <table className="min-w-full bg-white border border-gray-200">
                  ...
                  <tbody>
                    {products.map((product, index) => {
                      return (
                        <tr key={index} className="border-t hover:bg-gray-50" onClick={() => { setSelectedProduct(product); dispatch(toggleViewProductModal()) }}>
                          ...
                          <td>{product.price}</td>
                          <td>{product.stock}</td>
                          <td className="text-yellow-500">{product.ratings}</td>
                          ...
                          <td className="py-3 px-4 flex gap-2">
                            ...
                            <button className="text-white rounded-md cursor-pointer font-semibold bg-red-gradient px-2 py-1 font-semibold bg-red-gradient flex gap-2 items-center" onClick={(e) => {
                              e.stopPropagation();
                              setSelectedProduct(product);
            ...
            {!fetchingProducts && products.length > 0 && (
              <div className="flex justify-center mt-6 gap-4">
                <button onClick={() => setPage((prev) => Math.max(prev - 1, 1))} disabled={page === 1} className="px-4 py-2 bg-blue-500 hover:bg-blue-600 text-white rounded disabled:opacity-50">Previous</button>

                <span className="px-4 py-2 text-gray-700">Page {page}</span>

                <button onClick={() => setPage((prev) => prev + 1)} disabled={maxPage === page} className="px-4 py-2 bg-blue-500 hover:bg-blue-600 text-white rounded disabled:opacity-50">Next</button>
              </div>
            )}
          </div>
        </div>
