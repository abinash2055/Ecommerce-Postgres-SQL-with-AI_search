...
export const forgotPassword = (email) => async (dispatch) => {
  ...
  try {
    await axiosInstance.post(`/auth/password/forgot?frontendUrl=http://localhost:5174`, email).then((res) => {
      dispatch(authSlice.actions.forgotPasswordSuccess());
      toast.success(res.data.message);
    })
  } catch (error) {
    dispatch(authSlice.actions.forgotPasswordFailed());
    toast.error(error.response.data.message || "Admin cannot request for Reset Password....");
  }
};
...
export const resetPassword = (newData, token) => async (dispatch) => {
  ...
  try {
    ...
    toast.error(error.response.data.message || "Admin Failed to Reset Password....");
  }
};
...
export const updateAdminProfile = (data) => async (dispatch) => {
  ...
  try {
    ...
    toast.error(error.response.data.message || "Admin Failed to Update Profile....");
  }
};
...
export const updateAdminPassword = (data) => async (dispatch) => {
  ...
  try {
    ...
    toast.error(error.response.data.message || "Admin Failed to Update Password....");
  }
};
