      ...
      border-border rounded-2xl p-8 w-full max-w-2xl">
        ...
        <div className="flex items-center justify-between mb-8">
          <div className="flex items-center space-x-3">
            ...
            <h2 className="text-2xl font-bold text-foreground">AI Product Search</h2>
          </div>

          <button onClick={() => dispatch(toggleAIModal())} className="p-2 rounded-lg hover:bg-secondary transition-colors">
            <X className="w-5 h-5 text-foreground" />
          </button>
        </div>

        {/* Description */}
        <p className="text-muted-foreground mb-6">Describe what you're looking for and our AI will find the perfect products for you.</p>
        ...
        <div className="mt-6">
          ...
          <div className="flex flex-wrap gap-2">
            ...
            {exampleText.map((example) => (
              <button key={example} onClick={() => setUserPrompt(example)} className="px-3 py-1 bg-secondary text-foreground rounded-full text-sm hover:bg-primary hover:text-primary-foreground transition-colors">
                {example}
              </button>
            ))}
          </div>
