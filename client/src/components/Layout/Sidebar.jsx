        ...
        <nav className="p-6">
          <ul className="space-y-2">
            {menuItems.filter(Boolean).map(item => {
              return (
                <li key={item.name}>
                  <Link to={item.path} onClick={() => dispatch(toggleSidebar())} className="flex items-center space-x-3 rounded-lg glass-card  hover:glow-on-hover animate-smooth text-foreground hover:text-primary group">
