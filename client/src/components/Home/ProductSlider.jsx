...
const ProductSlider = ({ title, products }) => {
  ...
  const scroll = (direction) => {
    if (scrollRef.current) {
      ...
      scrollRef.current.scrollBy({ left: direction === "left" ? -scrollAmount : scrollAmount })
    }
  }
            ...
            return (
              <Link key={product.id} to={`/product/${product.id}`} className="flex-shrink-0 w-80 glass-card hover:glow-on-hover animate-smooth group">
                ...
                <div className="relative overflow-hidden rounded-lg mb-4">
                  ...
                  {/* Quick Add to Cart */}
                  <button onClick={(e) => handleAddToCart(product, e)} className="absolute bottom-3 right-3 p-2 glass-card hover:glow-on-hover animate-smooth opacity-0 group-hover:opacity-100 transition-opacity" disabled={product.stock === 0}>
